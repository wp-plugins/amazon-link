...
<p>However links can be created by using the keyword pair <code>\'%LINK_START%\'</code> and <code>\'%LINK_END%\'</code> with the subject of the link being placed between them. For example <code>\'%LINK_START%Amazon Product%LINK_END%\'</code>. The link produced will comply with whatever settings you have used, i.e. localised to the user\'s country or produce a multinational popup, it will also use the appropriate Amazon associate IDs.</p>
<p>There are a number of other keywords that are also localised these include: <code>\'%LINK_START%\'</code> - as described above, <code>\'%TLD%\'</code> the Top Level Domain to be used \'.co.uk\', \'.it\', \'.com\', etc.; <code>\'%MPLACE%\'</code> - the Amazon Market place to use \'GB\', \'IT\', \'US\', etc.; <code>\'%CC%\'</code> - the localised country code \'uk\', \'it\', \'us\'; <code>\'%TAG%\'</code> - The amazon associate tag to use.</p>
